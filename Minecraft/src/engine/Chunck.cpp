		...
		for (int z = 0; z < size; ++z)
			for (int x = 0; x < size; ++x)
			{
				...
				if(block.solid)
				{
					...
					{
						glm::ivec3 pos = m_position * size + glm::ivec3(x, y + 1, z);
						if (!World::BlockGenerated(pos) || !World::GetBlock(pos).solid || (World::GetBlock(pos).seeThrough && (World::GetBlock(pos).type != block.type || ( !World::GetBlock(pos).transparent || ! block.transparent))) )
						{
							std::vector<Mesh::Vertex> topFace = Cube::cubeTopFace(Block::size, (float)x, (float)y, (float)z, TexturesBlocks::Top(block.type));
